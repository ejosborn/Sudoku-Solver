# GUI.py
# This is a Sudoku Solving program with a GUI using pygame

import pygame
from solveForGUI import solveSudoku, validate
import time
...
class Grid:
    ...
    def __init__(self, rows, cols, width, height):  # Setting all initial variables
        self.rows = rows
        self.cols = cols
    ...
    def place(self, val):
        ...
        if self.cubes[row][col].value == 0:
            ...
            if validate(self.model, val, (row, col)) and solveSudoku(self.model):
                return True
            else:
    ...
    def draw(self, window):     # Drawing Elements of the Board
        # Grid Lines
        gap = self.width / 9
    ...
    def select(self, row, col):
        ...
        # Set Selected Cell to True
        self.cubes[row][col].selected = True
        self.selected = (row, col)

    def clear(self):    # clears value in cell
        row, col = self.selected
        if self.cubes[row][col].value == 0:
            self.cubes[row][col].set_temp(0)
...
class Cube:
    ...
    def __init__(self, value, row, col, width, height):     # sets initial elements of cell
        self.value = value
        self.temp = 0
        self.row = row
        self.col = col
    ...
    def draw(self, window):     # setting font of number in cell
